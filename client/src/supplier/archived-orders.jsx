...
function SupplierArchivedOrders() {
    const [orders, setOrders] = useState([]);
    const [isLoad, setIsLoad] = useState(false);
    useEffect(() => {
        setIsLoad(false);
        axios(`${API}/supplier/get-orders/archive`, {
            headers: {
    ...
    return (
        <div className="flex items-center justify-start flex-col w-full p-1">
            ...
            <div className="flex my-[10px] items-center xl:items-start justify-center xl:justify-start flex-col w-full mb-[10px]">
                <h1 className="text-[20px] md:text-[30px] sm:text-[40px] text-white animate-fade-down">ARXIVLAR</h1>
            </div>
            {/* Isload */}
            {!isLoad &&
...
export default SupplierArchivedOrders;