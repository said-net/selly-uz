...
function OperatorDashboard() {
    const [s, setS] = useState({ my_orders: 0, recontacts: 0, archiveds: 0, rejecteds: 0, success: 0, sendeds: 0, delivereds: 0, comming_balance: 0, balance: 0, payments: 0, comming_payments: 0 });
    // CLASSES
    ...
    useEffect(() => {
        axios(`${API}/operator/get-dashboard`, {
            headers: {
    ...
    return (
        <div className="flex items-center justify-start flex-col w-full relative">
            ...
            <div className="flex items-start justify-center w-full flex-wrap gap-3">
                {/* MY - ORDERS */}
                <Link to={'/operator/my-orders'} className={classCont + 'animate-delay-[50ms]'}>
                ...
                {/* RECONTACT - ORDERS */}
                <Link to={'/operator/recontacts'} className={classCont + 'animate-delay-[75ms]'}>
                ...
                {/* SUCCESS - ORDERS */}
                <Link to={'/operator/success'} className={classCont + 'animate-delay-[100ms]'}>
                    <div className={classIcon + 'bg-green-100 text-green-500'}>
                        <FaShoppingBasket />
                    </div>
                    <p className="absolute top-[1px] right-[20px]">Qadoqlashda</p>
                    <p className="text-[30px]">{s?.success?.toLocaleString()} <sub>ta</sub></p>
                </Link>
                {/* SENDED - ORDERS */}
                <Link to={'/operator/sended'} className={classCont + 'animate-delay-[125ms]'}>
                    <div className={classIcon + 'bg-indigo-100 text-indigo-500'}>
                        <FaTruckFast />
                    </div>
                    <p className="absolute top-[1px] right-[20px]">Yuborilganlar</p>
                    <p className="text-[30px]">{s?.sendeds?.toLocaleString()} <sub>ta</sub></p>
                </Link>
                {/* DELIVERED - ORDERS */}
                <Link to={'/operator/delivered'} className={classCont + 'animate-delay-[150ms]'}>
                ...
                {/* REJECT - ORDERS */}
                <Link to={'/operator/reject'} className={classCont + 'animate-delay-[175ms]'}>
                ...
                {/* REJECT - ORDERS */}
                <Link to={'/operator/archive'} className={classCont + 'animate-delay-[200ms]'}>
                    <div className={classIcon + 'bg-deep-orange-100 text-deep-orange-500'}>
                        <FaBoxArchive />
                    </div>
                    <p className="absolute top-[1px] right-[20px]">Arxivlanganlar</p>
                    <p className="text-[30px]">{s?.archiveds?.toLocaleString()} <sub>ta</sub></p>
                </Link>
                {/* COMMING - BALANCE */}
                <Link to={'/operator/finance'} className={classCont + 'animate-delay-[225ms]'}>
                ...
                {/* BALANCE */}
                <Link to={'/operator/finance'} className={classCont + 'animate-delay-[250ms]'}>
                    <div className={classIcon + 'bg-green-100 text-green-500'}>
                        <FaCreditCard />
                    </div>
                ...
                {/* COMMING - PAYMENT */}
                <Link to={'/operator/finance'} className={classCont + 'animate-delay-[275ms]'}>
                ...
                {/*  PAYMENTS */}
                <Link to={'/operator/finance'} className={classCont + 'animate-delay-[300ms]'}>
                    <div className={classIcon + 'bg-orange-100 text-orange-500'}>
                        <FaMoneyBill />
                    </div>
