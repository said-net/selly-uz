...
function AdminArchivedOrders() {
    ...
    useEffect(() => {
        setIsLoad(false);
        axios(`${API}/admin/get-archived-orders/${page}`, {
            headers: {
    ...
    document.title = `ARXIVLAR(${orders?.length})`
    // Selects
    function SelectAll() {
    ...
    return (
        <div className="flex items-center justify-start flex-col w-full relative">
            {/* INTRO */}
            <p className="uppercase text-white text-[20px] sm:text-[30px] animate-fade-down my-[10px]">ARXIVLAR</p>
            {/* FILTER */}
                    ...
                    orders?.filter(o => String(o?.operator_id)?.includes(filterOperator))?.map((o, i) => {
                        return (
                            <div key={i} className="flex items-center justify-start border-t-0 border border-r-0 border-gray-500">
                                ...
                                {/* PRODUCT */}
                                <p className="w-[150px] h-[50px] border-r flex items-center justify-center border-r-gray-500 text-[14px] text-center">
                                    {o?.title[17] ? o?.title?.slice(0, 17) + '...' : o?.title}
                                </p>
                                {/* PRODUCT IMAGE */}
