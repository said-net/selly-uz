...
function AdminInOperatorOrders() {
    ...
    useEffect(() => {
        setIsLoad(false);
        axios(`${API}/admin/get-in-operator-orders`, {
            headers: {
    ...
    document.title = `OPERATORLARDA(${orders?.length})`
    // Selects
    function SelectAll() {
    ...
    return (
        <div className="flex items-center justify-start flex-col w-full relative">
            {/* INTRO */}
            <p className="uppercase text-white text-[20px] sm:text-[30px] animate-fade-down my-[10px]">Operatorlarda</p>
            {/* FILTER */}
            <div className={`flex items-start justify-center flex-col w-[96%] animate-fade-up rounded-[4px_4px_0_0] gap-2 bg-white px-[10px] duration-300 p-[7px] z-[999]`}>
                ...
                {isLoad &&
                    <div className="flex items-center justify-center w-full ">
                        <Select label="Operator filteri" onChange={e => setFilterOperator(e)} value={filterOperator} color="indigo">
                            {operators?.map(o => (
                                <Option key={o.id} value={`${o.id}`}>ID: {o?.id} | {o.name} | {o?.phone} | <b className="text-indigo-500">{orders?.filter(order => order?.operator_id === o?.id)?.length}</b> ta</Option>
                            ))}
                        </Select>
                    </div>
                }
            ...
            <div className="flex items-start justify-start flex-col w-[96%] overflow-x-scroll bg-white p-[10px] rounded-[0_0_4px_4px] animate-fade-up">
                ...
                {isLoad && orders[0] &&
                    orders?.filter(o => String(o?.operator_id)?.includes(filterOperator))?.map((o, i) => {
                        return (
...
export default AdminInOperatorOrders;