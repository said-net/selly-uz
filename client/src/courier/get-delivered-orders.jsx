...
function CourierDeliveredOrders() {
    ...
    useEffect(() => {
        setIsLoad(false);
        axios(`${API}/courier/get-delivered-orders`, {
            headers: {
    ...
    return (
        <div className="flex items-center justify-start flex-col w-full relative pb-[50px]">
            {/* INTRO */}
            <p className="uppercase text-white text-[20px] sm:text-[30px] animate-fade-down my-[10px]">Yetkazilganlar</p>
            {/* SEARCH */}
                    ...
                    orders?.filter(o => String(o?.id)?.includes(search?.replace('S-', '')))?.map((o, key) => {
                        return (
                            <div key={key} className="flex flex-col items-start p-[5px] gap-1 justify-start w-[90%] bg-white md:w-[300px] rounded-[10px] hover:bg-gray-200 cursor-pointer animate-fade-up">
                                ...
                                {/* STATUS */}
                                <Chip color="green" value={"Yetkazilgan"} className="w-full flex items-center justify-center font-normal" />
                                {/* IMAGE */}
...
export default CourierDeliveredOrders;